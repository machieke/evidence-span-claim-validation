...
def detect_image_ocr_spans(config: PipelineConfig, source_id: Optional[str] = None) -> SpanDetectionResult:
    paths = config.jsonl_paths()
    evidence_by_id = {
        evidence.evidence_id: evidence
        for _, evidence in read_jsonl_records(paths["evidence"], EvidenceRecord)
        if evidence.source_modality == "image" and evidence.evidence_type == "ocr_text_span"
    }
    existing_span_ids = existing_values(paths["spans"], "span_id")
    created = 0
    skipped = 0
            ...
            for segment in split_sentences(evidence.text):
                ...
                append_jsonl(
                    ...
                    SpanRecord(
                        ...
                        label="claim_bearing",
                        score=score,
                        detector={"name": "image_ocr_rules_v1", "version": "0.1.0"},
                        risk_flags=risk_flags_for_text_segment(segment.text, evidence.provenance, evidence.risk_flags),
                    ),
                )
                existing_span_ids.add(span_id)
