...
def test_validate_claims_quarantines_missing_chat_provenance(tmp_path: Path):
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        assert validations[0]["validator_version"] == "deterministic.v9"
...
def test_validate_claims_quarantines_missing_audio_provenance(tmp_path: Path):
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        assert validations[0]["warnings"] == [
            "missing_audio_source_duration_provenance",
...
def test_validate_claims_quarantines_audio_timestamp_out_of_bounds(tmp_path: Path):
    with runner.isolated_filesystem(temp_dir=tmp_path):
        init = runner.invoke(app, ["init"])
        assert init.exit_code == 0
        ...
        append_jsonl(
            Path("data/jsonl/claims.raw.jsonl"),
            RawClaimRecord(
                claim_id="claim_audio_out_of_bounds",
                source_id="src_audio_1",
                source_modality="audio",
                evidence_id="ev_audio_1",
                source_faithful_claim="The speaker asserted that Hope departed at 09:00.",
        ...
        validations = [payload for _, payload in read_jsonl(Path("data/jsonl/validations.jsonl"))]
        quarantined = [payload for _, payload in read_jsonl(Path("data/jsonl/quarantine.jsonl"))]
        assert validations[0]["errors"] == ["audio_timestamp_out_of_source_bounds"]
        assert validations[0]["validator_version"] == "deterministic.v9"
        assert quarantined[0]["reason_codes"] == ["audio_timestamp_out_of_source_bounds"]
...
def test_validate_claims_allows_attribution_entities_from_provenance(tmp_path: Path):
    with runner.isolated_filesystem(temp_dir=tmp_path):
        init = runner.invoke(app, ["init"])
        assert init.exit_code == 0
        ...
        claim = RawClaimRecord(
            claim_id="claim_attributed_to_display_name",
            source_id="src_chat_1",
            source_modality="chat",
            span_id="span_1",
            evidence_id="ev_msg_1",
            source_faithful_claim="Bob asserted that Hope had three masts.",
            subject="Hope",
            predicate="had",
            object="three masts",
            modality="asserted",
        ...
        assert "claims_quarantined=0" in result.stdout

        validations = [payload for _, payload in read_jsonl(Path("data/jsonl/validations.jsonl"))]
        assert validations[0]["status"] == "accepted_extracted"
        assert validations[0]["errors"] == []
        assert validations[0]["warnings"] == []
        assert validations[0]["metadata"]["validation"]["introduced_entities"] == []
...
def test_validate_claims_quarantines_unit_mismatch(tmp_path: Path):
    with runner.isolated_filesystem(temp_dir=tmp_path):
        init = runner.invoke(app, ["init"])
        assert init.exit_code == 0
        ...
        evidence = EvidenceRecord(
            ...
            text="The mast is 12 meters tall.",
            provenance={"page": 1, "block_id": "pdf_block_1", "extractor": "pypdf"},
        )
        claim = RawClaimRecord(
            claim_id="claim_unit_mismatch",
            source_id="src_pdf_1",
            source_modality="pdf",
            evidence_id="ev_pdf_1",
            source_faithful_claim="The document states that the mast is 12 feet tall.",
            subject="mast",
            predicate="height",
            object="12 feet",
            modality="asserted",
            evidence_text="The mast is 12 meters tall.",
...
def test_validate_claims_enforces_pdf_provenance_requirements(tmp_path: Path):
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        assert validations["claim_pdf_bad"]["validator_version"] == "deterministic.v9"
